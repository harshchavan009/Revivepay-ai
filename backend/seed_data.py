...
def seed_database(force_reseed: bool = False):
    ...
    # ==========================================
    # 3b. DETERMINISTIC HIGH-VALUE CASE (₹85,000 — SIMULATION — AWAITING_APPROVAL)
    # ==========================================
    highvalue_payment = Payment(
        payment_id="pay_demo_highvalue_85k",
        merchant_id=merchant.merchant_id,
        customer_id=demo_cust.customer_id,
        provider="razorpay",
        provider_payment_id="pay_rzp_test_highvalue_85k",
        order_id="order_rzp_highvalue_85k",
        amount=85000.0,
        currency="INR",
        payment_method="card",
        status="FAILED",
        failure_code="BANK_TIMEOUT",
        failure_reason="Core issuer switch failure during high-value authorization",
        failure_category="temporary_bank_failure",
        retry_count=0,
        max_retry_count=2,
        source="SIMULATION",
        ...
        updated_at=datetime.datetime.utcnow() - datetime.timedelta(minutes=15)
    )
    db.add(highvalue_payment)
    db.commit()
    db.refresh(highvalue_payment)

    highvalue_case = RecoveryCase(
        case_id="RV-DEMO-HIGHVALUE",
        payment_id=highvalue_payment.payment_id,
        customer_id=demo_cust.customer_id,
        source="SIMULATION",
        source_description="Synthetic event generated by RevivePay",
        case_type="PAYMENT_FAILURE",
        ...
        risk_level="CRITICAL",
        risk_factors={
            "transaction_value_factor": 95.0,
            "recovery_likelihood_factor": 25.0,
            "customer_history_factor": 15.0,
        ...
        ai_confidence=0.88,
        evidence=[
            "Enterprise tier customer Vikramaditya Sharma",
            "Transaction value ₹85,000 exceeds ₹50,000 policy auto-execution threshold",
            "AI recommended retry_payment, but deterministic policy requires operator sign-off"
        ],
        recommended_action="retry_payment",
        reasoning_summary="High-value enterprise transaction exceeds ₹50,000 automatic limit. Guardrail mandates human-in-the-loop signoff regardless of AI confidence.",
        policy_status="REVIEW_REQUIRED",
        policy_checklist=[
            {"rule": "action_whitelisted", "description": "Action 'retry_payment' permitted by policy", "passed": True, "details": "Permitted"},
            {"rule": "auto_action_amount_limit", "description": "Amount ₹85,000 <= ₹50,000 auto limit", "passed": False, "details": "Exceeds ₹50,000 threshold"},
            {"rule": "rbi_tat_guideline_compliance", "description": "RBI Turn Around Time check", "passed": True, "details": "On Track"}
        ],
        approval_required=True,
        approval_status="PENDING",
        execution_status="IDLE",
        ...
        outcome_verified=False,
        recovered_amount=0.0,
        tat_deadline=datetime.datetime.utcnow() + datetime.timedelta(days=5),
        tat_status="ON_TRACK",
        accrued_compensation_inr=0.0,
        created_at=datetime.datetime.utcnow() - datetime.timedelta(minutes=15),
        updated_at=datetime.datetime.utcnow() - datetime.timedelta(minutes=15)
    )
    db.add(highvalue_case)
    db.commit()

    db.add(Approval(
        approval_id="appr_demo_highvalue_85k",
        case_id=highvalue_case.case_id,
        requested_action="retry_payment",
        risk_level="CRITICAL",
        requested_at=highvalue_case.created_at + datetime.timedelta(seconds=2),
        requested_by="Policy Gateway",
        decision="PENDING",
        decision_reason="Transaction value ₹85,000 exceeds ₹50,000 threshold. Operator approval required."
    ))

    # 3c. DETERMINISTIC RETRY EXHAUSTION CASE (₹6,200 — SIMULATION — POLICY BLOCK -> ESCALATED)
    exhausted_payment = Payment(
        payment_id="pay_demo_exhausted_6200",
        merchant_id=merchant.merchant_id,
        customer_id=demo_cust.customer_id,
        provider="razorpay",
        provider_payment_id="pay_rzp_test_exhausted_6200",
        order_id="order_rzp_exhausted_6200",
        amount=6200.0,
        currency="INR",
        payment_method="card",
        status="FAILED",
        failure_code="PERSISTENT_DECLINE",
        failure_reason="Issuer declined recurring attempt: insufficient balance across window",
    ...
    db.add(exhausted_payment)
    db.commit()

    exhausted_case = RecoveryCase(
        case_id="RV-DEMO-RETRY-EXHAUSTION",
        payment_id=exhausted_payment.payment_id,
        customer_id=demo_cust.customer_id,
        source="SIMULATION",
        source_description="Synthetic event generated by RevivePay",
        case_type="PAYMENT_FAILURE",
        amount_at_risk=6200.0,
        risk_score=78.0,
        risk_level="HIGH",
        risk_factors={
            "transaction_value_factor": 60.0,
            "recovery_likelihood_factor": 10.0,
            "customer_history_factor": 20.0,
            "failure_severity_factor": 45.0,
            "weights": {"value": 0.35, "recovery": 0.25, "history": 0.20, "severity": 0.20}
        },
        root_cause="Insufficient Funds",
        ai_confidence=0.86,
        evidence=[
            "Retry #1 executed at T+5m — Issuer Response: DECLINE",
            "Retry #2 executed at T+20m — Issuer Response: DECLINE",
            "Max retries limit (2/2) reached. Deterministic policy triggered automation block."
        ],
        recommended_action="stop_recovery",
        reasoning_summary="Maximum retries reached (2/2). Automation stopped under policy guardrail to prevent spamming customer account. Escalated to operator review queue.",
        policy_status="REJECTED",
        policy_checklist=[
            {"rule": "action_whitelisted", "description": "Action permitted by policy", "passed": True, "details": "Permitted"},
            {"rule": "max_retries_limit", "description": "Retry attempt count check (2/2)", "passed": False, "details": "Max retries 2 reached — further retries permanently blocked"},
            {"rule": "rbi_tat_guideline_compliance", "description": "RBI Turn Around Time check", "passed": True, "details": "On Track"}
        ],
        ...
        outcome_verified=False,
        recovered_amount=0.0,
        tat_deadline=datetime.datetime.utcnow() + datetime.timedelta(days=4),
        tat_status="ON_TRACK",
        accrued_compensation_inr=0.0,
        created_at=datetime.datetime.utcnow() - datetime.timedelta(minutes=30),
        updated_at=datetime.datetime.utcnow() - datetime.timedelta(minutes=5)
    )
    db.add(exhausted_case)
    db.commit()
    ...
    all_logs = db.query(AuditLog).order_by(AuditLog.timestamp.asc(), AuditLog.audit_id.asc()).all()
